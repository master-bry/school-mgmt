...
class AssistantHeadController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // Query 1: classes + student counts
        $classes = DB::select("
            SELECT c.id, c.name,
        ...
        if ($classIds) {
            // Query 2: today's attendance by class
            $todayAttCounts = DB::select("
                SELECT u.class_id, a.status, count(*) as count
                FROM attendances a
                JOIN users u ON u.id = a.student_id
                WHERE a.school_id = ? AND a.date = ?::date AND u.class_id IN (" . implode(',', array_fill(0, count($classIds), '?')) . ")
                GROUP BY u.class_id, a.status
            ", array_merge([$schoolId, $today], $classIds));
            $todayAttGrouped = collect($todayAttCounts)->groupBy('class_id');

            // Query 3: avg grades by class
            $avgGrades = DB::select("
            ...
            $todayAttGrouped = collect();
            $avgGrades = collect();
        }

        $classPerformance = array_map(fn($c) => [
            'name'           => $c->name,
            'students_count' => $c->students_count,
            'present_today'  => ($todayAttGrouped->get($c->id)?->where('status', 'present')->sum('count')) ?? 0,
            'absent_today'   => ($todayAttGrouped->get($c->id)?->where('status', 'absent')->sum('count')) ?? 0,
            'avg_grade'      => round($avgGrades->get($c->id, 0), 1),
        ], $classes);

        // Query 4: aggregate counts
        $agg = DB::selectOne("
            SELECT
              ...
              (SELECT count(*) FROM grade_submissions WHERE school_id = ? AND status = 'pending')  AS pending_submissions,
              (SELECT count(*) FROM fees WHERE school_id = ? AND status IN ('pending','overdue'))  AS pending_fees
        ", [$schoolId, $schoolId, $schoolId, $schoolId]);

        // Query 5: today's timetable
        $todayTimetable = Timetable::where('school_id', $schoolId)
            ->where('day', strtolower(now()->format('l')))
            ->with(['class', 'subject', 'teacher'])
            ->orderBy('start_time')->get();

        // Query 6: recent exams
        $recentExams = Exam::where('school_id', $schoolId)
            ->with('subject', 'class')->latest('exam_date')->take(5)->get();
        ...
        return response()->json([
            ...
            'pending_submissions'   => $agg->pending_submissions,
            'pending_fees'          => $agg->pending_fees,
    ...
    public function students()
    {
        return response()->json(
            User::where('school_id', $this->schoolId())->where('role', 'student')
                ->with('class')->get(['id', 'name', 'email', 'class_id', 'phone', 'date_of_birth'])
        );
    }
    ...
    {
        return response()->json(
            User::where('school_id', $this->schoolId())->where('role', 'teacher')
    ...
    public function getPendingFees()
    {
        $fees = Fee::with('student')
            ...
            ->where('approval_status', 'pending_cashier')
            ->orderBy('created_at', 'desc')
            ->get();
        return response()->json($fees);
    }

    public function reviewFee(Request $request, $id)
    {
        $data = $request->validate([
